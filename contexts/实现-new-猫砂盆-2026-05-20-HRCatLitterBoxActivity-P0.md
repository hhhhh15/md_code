...
这个索引哪里来的啊？我想要看一下这个属性字段specs?.enum的数据是不是

这个方法返回的是对应的字符串，在任务面板操作的方法中索引对应
 val rawTaskType = currentTaskObj[DevicePropertyConstants.TASK_TYPE]
        taskTypeCode = getTaskTypeCodeFromSpecs(rawTaskType)
这个enumList从全量物模型中获取的必须要和属性接口的task_type一一对应啊。，要不然这里return enumList[index]返回的string字符串就错了
...
```
步骤1: 开口朝上运行中 (MAINTAINING + RUNNING)，（5，1）
  → 取消按钮变"复位"，灰色不可点
  → 暂停/继续按钮正常

步骤2: 已完成开口朝上 (MAINTAINING + IDLE) ，（5，3）
  → 复位按钮可点
  → 暂停/继续按钮隐藏
  → 中间文字："已完成开口朝上"

步骤3: 开口复位中 (RESETTING + RUNNING)，（6，1）
  → 复位按钮隐藏
  → 暂停/继续按钮正常显示
```
...
```java
 private fun handleDeviceTaskStatus(result: Map<String, Any?>) {
        try {
            ...
            if (
                (taskType == DeviceTaskType.CLEANING || taskType == DeviceTaskType.EMPTYING || taskType == DeviceTaskType.SMOOTHING || taskType == DeviceTaskType.RESETTING || taskType == DeviceTaskType.REFILLING || taskType == DeviceTaskType.MAINTAINING) &&
                (taskStatus == DeviceTaskStatus.RUNNING || taskStatus == DeviceTaskStatus.PAUSED || taskStatus == DeviceTaskStatus.COMPLETED)
                //100A+开口朝上，task_type=5 && task_status=3，是已完成开口朝上，要显示任务面板sllCurrentTask
            )
                {
                ...
                mBind.tvCancelDesc.text = getString(R.string.catlitterbox_button_cancel) // 默认重置为"取消"

                if (taskType == DeviceTaskType.MAINTAINING && taskStatus == DeviceTaskStatus.COMPLETED) {
                    // 步骤2.已完成开口朝上（task_type=5, task_status=3）→ 复位可点，暂停/继续隐藏，取消按钮文字图片改变复位
                ...
                } else if (taskType == DeviceTaskType.RESETTING) {
                    // 步骤3：开口复位中 → 复位按钮隐藏，暂停/继续正常显示
                    mBind.llCancel.visibility = View.GONE
                } else if (taskType == DeviceTaskType.MAINTAINING) {
                    // 步骤1：开口朝上运行/暂停中（task_type=5, task_status=1 or 2）→ 复位灰色不可点
                    mBind.tvCancelDesc.text = getString(R.string.catlitterbox_text_reset)
                ...
                if (isCatInside || isCatNear) {
                    ...
                    if (taskStatus == DeviceTaskStatus.PAUSED) {
                        ...
                        mBind.tvPauseContinue.text = getString(R.string.catlitterbox_button_continue)
                        mBind.ivPauseContinue.setImageResource(R.mipmap.ic_equipment_continue)
                    } else if(taskStatus!=DeviceTaskStatus.COMPLETED){
                        // 运行中
                        when (taskType ) {
                            ...
                            DeviceTaskType.MAINTAINING->mBind.tvEventDesc.text =getString(R.string.catlitterbox_text_status_open_up)
                ...
                when (taskType) {
                    ...
                    DeviceTaskType.MAINTAINING->mBind.ivEventIcon.setImageResource(R.mipmap.ic_quick_open_upwards)
...
6.17:猫咪靠近和猫咪进入，会影响任务状态，任务会暂停的，所以任务面板里面有猫咪靠近和猫咪进入的分支，暂停/继续按钮是变灰不能点击的，并且猫咪进入现在是暂停状态，所以在这段代码上面，5，3;集便箱门打开状态，判断那些按钮显示哪些不显示。猫咪进入分支会对有暂停/继续按钮进行处理


---

# 实现5.余量详情页 HRCatLitterBoxRemainingActivity

## 1. 页面入口

从 HRCatLitterBoxActivity 的余量区域点击进入，通过 `type` 参数区分展示砂仓还是集便箱：

```java
// HRCatLitterBoxActivity 中的点击事件
listOf(mBind.rlSurplus1 to 0, mBind.rlSurplus2 to 1).forEach { (view, type) ->
    view.singleClick {
        val intent = Intent(this, DeviceCatLitterBoxSurplusActivity::class.java)
        intent.putExtra("device_model", deviceModel)
        intent.putExtra("type", type)  // 0=砂仓, 1=集便箱
        startActivity(intent)
    }
}
```

## 2. updateRemainingUI() — 余量状态 UI 控制

根据 `type` 展示不同余量状态，优先级：**未安装 > 物模型常规状态**。

### 2.1 type=0 砂仓（litter bin）

| 优先级 | 条件 | 显示文字 | 文字颜色 | 图标 |
|--------|------|----------|----------|------|
| 1（最高） | `!isToiletInPlace` | 砂仓未安装 | 红色 ff2742 | ic_clb_litter_bin_not_installed_75v |
| 2 | `toiletRemainStatus = true` | 充足 | 默认色 | ic_clb_litter_bin_sufficient_75v |
| 2 | `toiletRemainStatus = false` | 不足 | 红色 ff2742 | ic_clb_litter_bin_insufficient_75v |

### 2.2 type=1 集便箱（waste bin）

| 优先级 | 条件 | 显示文字 | 文字颜色 | 图标 |
|--------|------|----------|----------|------|
| 1（最高） | `!isWasteBinInPlace` | 集便箱未安装 | 红色 ff2742 | ic_clb_feces_box_not_installed_75v |
| 2 | `wasteBinStatus = true` | 已满，余量不足 | 红色 ff2742 | ic_clb_feces_box_full_75v |
| 2 | `wasteBinStatus = false` | 正常 | 默认色 | ic_clb_feces_box_sufficient_75v |

> 注意：集便箱的 `wasteBinStatus = true` 表示**空间不足（已满）**，与砂仓的 `toiletRemainStatus = true` 表示充足**语义相反**。
